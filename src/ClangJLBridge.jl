...
# Arguments
- `config::Dict`: JMake configuration dictionary (from TOML)
...
function generate_bindings_clangjl(config::Dict, lib_path::String, headers::Vector{String})
    ...
    try
        ...
        add_jmake_metadata(output_file, module_name, lib_path, headers)
...
"""
    add_jmake_metadata(file_path::String, module_name::String, lib_path::String, headers::Vector{String})

Add JMake metadata comments to the generated bindings file.
"""
function add_jmake_metadata(file_path::String, module_name::String, lib_path::String, headers::Vector{String})
    if !isfile(file_path)
        return
    end
    ...
    header = """
    # Auto-generated Julia bindings by JMake + Clang.jl
    # Generated: $(Dates.format(now(), "yyyy-mm-dd HH:MM:SS"))
    # Module: $module_name
    # Library: $(basename(lib_path))
    # Headers: $(join([basename(h) for h in headers], ", "))
    #
    # This file was generated automatically by JMake using Clang.jl.
...
Convenience function to generate bindings directly from JMake config file.
...
# Examples
```julia
generate_from_config("jmake.toml")
generate_from_config("jmake.toml", lib_path="custom/libfoo.so")
```
"""
function generate_from_config(config_file::String; lib_path::String="", headers::Vector{String}=String[])
    ...
    if !isfile(lib_path)
        error("Library not found: $lib_path\nCompile the project first with JMake.compile()")
    end
