...
# Complements the JMake LLVMake compiler by providing advanced binary introspection and wrapping
...
function load_wrapper_config(config_file::String)::WrapperConfig
    if !isfile(config_file)
        ...
        if isfile("jmake.toml")
            config = create_wrapper_config_from_stage1()
            save_wrapper_config(config, config_file)
            println("📝 Created wrapper config from JMake LLVMake: $config_file")
        else
...
function create_wrapper_config_from_stage1()::WrapperConfig
    # Load stage 1 config
    stage1_data = TOML.parsefile("jmake.toml")
...
function main()
    if length(ARGS) == 0
        println("""
        JMake JuliaWrapItUp - Universal Binary Wrapper Generator
