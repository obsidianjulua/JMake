...
const CONFIG_CACHE = Dict{String, ConfigurationManager.JMakeConfig}()  # path => config
...
function init_template_cache!()
    ...
    TEMPLATE_CACHE[:cpp_project] = Dict(
        "dirs" => ["src", "include", "julia", "build", "test", ".jmake_cache"],
        "files" => Dict(
            ".jmake_project" => "",
            ".gitignore" => "build/\n.jmake_cache/\n*.so\n*.o\n*.ir\n*.bc\n"
        )
    ...
    TEMPLATE_CACHE[:binary_project] = Dict(
        "dirs" => ["lib", "bin", "julia_wrappers", ".jmake_cache"],
        "files" => Dict(
            ".jmake_project" => "",
            ".gitignore" => ".jmake_cache/\njulia_wrappers/\n"
        )
    )
        ...
        if !isnothing(discovery_results)
            ...
            if haskey(discovery_results, :ast) && discovery_results[:ast][:success]
                ...
                if haskey(ast_data, :graph) && !isnothing(ast_data[:graph])
                    config.discovery["dependency_graph_file"] = ".jmake_cache/dependency_graph.json"
                end
            end
...
function main()
    println("="^70)
    println("JMake Setup Daemon Server")
    println("Port: $PORT")
    println("="^70)
    ...
    println("  • validate_config(config='jmake.toml')")
