#!/usr/bin/env julia
# Build custom sysimage for JMake
# This creates a compiled sysimage with JMake precompiled for faster startup
...
println("🚀 Building JMake sysimage...")
println("=" ^ 60)

# Configuration
build_dir = @__DIR__
project_dir = dirname(build_dir)  # Parent directory (JMake root)
sysimage_path = joinpath(build_dir, "JMakeSysimage.so")
...
try
    # Create sysimage with JMake and all dependencies precompiled
    create_sysimage(
        [:JMake, :TOML, :JSON, :Dates, :Libdl];
    ...
    println("\nOr create an alias:")
    println("  alias jmake='julia -J $sysimage_path'")
    println("=" ^ 60)
