...
# Part of the JMake build system
...
struct LLVMJuliaCompiler
    ...
    function LLVMJuliaCompiler(config_file::String="jmake.toml")
...
function compile_project(compiler::LLVMJuliaCompiler;
    ...
    println("🚀 JMake LLVMake - C++ to Julia Compiler")
    println("="^50)
...
function main()
    if length(ARGS) == 0
        println("""
        JMake LLVMake - LLVM/Clang to Julia Compiler
    ...
    elseif command == "compile-file"
        ...
        cpp_file = ARGS[2]
        config_file = length(ARGS) >= 3 ? ARGS[3] : "jmake.toml"
    ...
    elseif command == "info"
        ...
        println("JMake Project Information:")
        println("="^50)
...
export optimize_and_link_ir, compile_ir_to_shared_lib
