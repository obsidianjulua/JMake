#!/usr/bin/env julia
# Discovery.jl - JMake Discovery Pipeline
# Orchestrates: file scanning, binary detection, AST dependency walking, config generation
# Entry point: JMake.discover() - called after Templates.jl plants the structure
...
"""
    discover(target_dir::String=pwd(); force::Bool=false) -> JMakeConfig

Main discovery pipeline - scans project and generates configuration.

# Process:
1. Check for .jmake_project marker
2. Scan all files and categorize
3. Detect and analyze binaries
4. Walk AST dependencies using clang
5. Generate jmake.toml with hardcoded LLVM paths
6. Return configuration

# Arguments
- `target_dir`: Project directory (default: current directory)
- `force`: Force rediscovery even if jmake.toml exists

# Returns
- `JMakeConfig`: Complete project configuration
"""
function discover(target_dir::String=pwd(); force::Bool=false)
    println("🔍 JMake Discovery Pipeline")
    println("="^70)
    println("📁 Target: $target_dir")
    println()

    # Check for marker
    if !check_marker(target_dir) && !force
        error("Not a JMake project. Run Templates.jl first or use force=true")
    end

    # Check if already configured
    config_path = joinpath(target_dir, "jmake.toml")
    if isfile(config_path) && !force
        println("⚠️  jmake.toml already exists!")
    ...
    println("🚀 Next: julia -e 'using JMake; JMake.compile()'")
    println("="^70)
...
Check if directory has .jmake_project marker.
"""
function check_marker(target_dir::String)
    marker_path = joinpath(target_dir, ".jmake_project")
    return isfile(marker_path)
end
...
function scan_all_files(root_dir::String)
    ...
    for (root, dirs, files_list) in walkdir(root_dir)
        # Skip build/cache directories
        filter!(d -> !in(d, ["build", ".git", ".cache", "node_modules", ".jmake_cache"]), dirs)
...
function walk_dependencies(root_dir::String, scan::ScanResults, include_dirs::Vector{String})
    ...
    json_path = joinpath(root_dir, ".jmake_cache", "dependency_graph.json")
...
"""
    generate_config(root_dir, scan, binaries, include_dirs, dep_graph) -> JMakeConfig

Generate JMake configuration from discovery results.
"""
function generate_config(root_dir::String, scan::ScanResults, binaries::Vector{BinaryInfo},
    ...
    config = ConfigurationManager.JMakeConfig(
        ...
        Dict{String,Any}(
            ...
            "dependency_graph_file" => isnothing(dep_graph) ? "" : ".jmake_cache/dependency_graph.json"
        ),
        ...
        # LLVM configuration (HARDCODED)
        Dict{String,Any}(
            "root" => JMAKE_LLVM_ROOT,
            "use_jmake_llvm" => true,
            "isolated" => true,
            "tools" => Dict(
                "clang" => "$JMAKE_LLVM_ROOT/tools/clang++",
                "llvm_config" => "$JMAKE_LLVM_ROOT/tools/llvm-config",
                "llvm_link" => "$JMAKE_LLVM_ROOT/tools/llvm-link",
                "opt" => "$JMAKE_LLVM_ROOT/tools/opt",
                "llc" => "$JMAKE_LLVM_ROOT/tools/llc",
                "llvm_nm" => "$JMAKE_LLVM_ROOT/tools/llvm-nm",
                "llvm_ar" => "$JMAKE_LLVM_ROOT/tools/llvm-ar"
            )
        ),
        # Target
        Dict{String,Any}(
