...
# Isolates and manages the JMake-specific LLVM 20.1.2 installation
# Ensures all JMake operations use the local toolchain without polluting system environment
...
function get_jmake_llvm_root()
    ...
    # We need to go up one level to /home/grim/.julia/julia/JMake
    # Then access LLVM subdirectory
    jmake_dir = dirname(@__DIR__)  # Go from src/ to JMake/
    llvm_root = joinpath(jmake_dir, "LLVM")

    if !isdir(llvm_root)
        error("JMake LLVM installation not found at: $llvm_root\n" *
              "Expected location: /home/grim/.julia/julia/JMake/LLVM")
    end
...
"""
    init_toolchain(;isolated::Bool=true) -> LLVMToolchain
...
"""
function init_toolchain(;isolated::Bool=true)
    llvm_root = get_jmake_llvm_root()

    println("🔧 Initializing LLVM Toolchain")
    ...
    # Discover tools
    tools = discover_llvm_tools(llvm_root)
    println("   Tools: $(length(tools)) discovered")
